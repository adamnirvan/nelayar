"""
Usage: python parse_forecast.py --date YYYY-MM-DD [--lat-min -11] [--lat-max 6] [--lon-min 95] [--lon-max 141]
Output: JSON array of 10 objects, one per day offset (D+1..D+10):
  [{ day_offset, forecast_date, geojson: FeatureCollection, sst_min, sst_max, chl_threshold, confidence }, ...]
"""
...
def fetch_cmems_forecast(base_date, day_offset, lat_min, lat_max, lon_min, lon_max):
    ...
    return sst, chl, lats, lons, res, date_str


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--date', required=True, help='Base date (YYYY-MM-DD); forecast is D+1 to D+10')
    ...
    for day_offset in range(1, 11):
        try:
            sst, chl, lats, lons, res, date_str = fetch_cmems_forecast(
                base_date, day_offset, args.lat_min, args.lat_max, args.lon_min, args.lon_max
            )
