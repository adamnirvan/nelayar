    ...
    return (
        <motion.div
            ...
            className="
                ...
                md:absolute md:top-0 md:bottom-auto md:h-full md:max-h-none md:w-[380px] md:rounded-none md:shadow-2xl md:p-6
            "
            ref={(ref) => {
                if (ref) {
        ...
        >
            ...
            <div className="mb-5 flex shrink-0 items-center justify-between gap-2 pointer-events-auto">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {(!isThisZone || nav.status === 'idle' || nav.status === 'error') && (
                        <button
                            type="button"
                            onClick={handleNavigate}
                            disabled={!center || nav.status === 'planning'}
                            className="glass-panel flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold text-gray-900 border border-white/50 shadow-sm hover:bg-white/40 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 transition-all"
                        >
                            <svg className="h-3.5 w-3.5 text-slate-700" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    ...
                    {isThisZone && nav.status === 'planning' && (
                        <button disabled className="glass-panel flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold text-gray-500 border border-white/20 shadow-sm cursor-wait animate-pulse">
                    ...
                    {isThisZone && nav.status === 'planned' && (
                        <button 
                            ...
                            className="flex items-center gap-1.5 rounded-full bg-green-600 px-3 py-1.5 text-xs font-bold text-white shadow-md hover:bg-green-700 active:scale-95 transition-all"
                        >
                    ...
                    {isThisZone && nav.status === 'active' && (
                        <button 
                            ...
                            className="flex items-center gap-1.5 rounded-full bg-red-50 border border-red-200 px-3 py-1.5 text-xs font-bold text-red-600 shadow-sm hover:bg-red-100 active:scale-95 transition-all"
                        >
                            Akhiri
                        </button>
                    )}
                    ...
                    <button 
                        type="button"
                        onClick={onClose} 
                        className="text-2xl font-bold text-gray-400 transition-colors hover:text-red-500 ml-1"
            ...
            >
               ...
               {center && (
                ...
                {isThisZone && nav.status === 'error' && nav.error && (
                    <div className="glass-inset mb-4 rounded-xl bg-red-500/10 border border-red-200 p-3 text-xs font-semibold text-red-900">
                        ⚠️ {nav.error}
                    </div>
                )}
                ...
                <div className="mt-2">
                    ...
                    <div className="space-y-3">
                        {props?.ikan_cocok?.map((ikan: any, idx: number) => {
                            const imgPath = ikan.image_path ? `/${ikan.image_path}` : '/fish/default.png';
                            return (
                                <div key={idx} className="glass-inset flex items-center rounded-xl p-3">
                                    ...
                                    <div className="ml-3 flex-grow min-w-0">
                                        <h4 className="text-xs font-bold text-gray-800 truncate">
                                            {ikan.nama_lokal}
                                            {ikan.nama_lain && <span className="font-normal text-gray-400"> / {ikan.nama_lain}</span>}
                                        </h4>
                                        <p className="text-[11px] italic text-slate-500 truncate">{ikan.nama_ilmiah}</p>
                                        <div className="mt-1 flex items-center">
                                            <div className="mr-2 h-1.5 w-full rounded-full bg-white/40">
                                                <div className="h-1.5 rounded-full bg-slate-600" style={{ width: `${ikan.confidence * 100}%` }}></div>
                                            </div>
                                            <span className="text-[10px] font-bold text-gray-500">{Math.round(ikan.confidence * 100)}%</span>
                                        </div>
                                    </div>
                                </div>
                            );
